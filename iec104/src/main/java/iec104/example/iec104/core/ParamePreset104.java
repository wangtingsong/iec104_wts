...
public class ParamePreset104 {
	...
	public String single_parmeter(int[] infoElement, int num, int tI, int sQ) {
		...
		string += "\n";
		string += "参数设置对象信息值：";
		string += Iec104Util.toHexString(infoElement[3]);
		string += "  ";
		string += Iec104Util.toHexString(infoElement[4]);
		string += "  ";
		string += Iec104Util.toHexString(infoElement[5]);
		string += "  ";
		string += Iec104Util.toHexString(infoElement[6]);
		string += "\n";
	...
	public String multi_parmeter(int[] infoElement, int num, int tI, int sQ) {
		...
		for (int i = 0; i < num; i++) {
			...
			string += "\n";
			string += "参数设置对象信息值：";
			string += Iec104Util.toHexString(infoElement[i * 7 + 3]);
			string += "  ";
			string += Iec104Util.toHexString(infoElement[i * 7 + 4]);
			string += "  ";
			string += Iec104Util.toHexString(infoElement[i * 7 + 5]);
			string += "  ";
			string += Iec104Util.toHexString(infoElement[i * 7 + 6]);
			string += "\n";
		}
		return string;
	}
	...
	public String activate_single_parmeter(int[] infoElement, int num, int tI,
		...
		string += Iec104Util.toHexString(infoElement[7]);
		string += "  ";
